import json

states_and_uts = [
  # 28 States
  {
    "id": "andhra-pradesh",
    "name": "Andhra Pradesh",
    "capital": "Amaravati",
    "region": "Southern India",
    "total_places": 18,
    "coordinates": {"lat": 15.9129, "lng": 79.7400},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "arunachal-pradesh",
    "name": "Arunachal Pradesh",
    "capital": "Itanagar",
    "region": "Northeastern India",
    "total_places": 12,
    "coordinates": {"lat": 28.2180, "lng": 94.7278},
    "thumbnail_url": "https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "assam",
    "name": "Assam",
    "capital": "Dispur",
    "region": "Northeastern India",
    "total_places": 16,
    "coordinates": {"lat": 26.2006, "lng": 92.9376},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "bihar",
    "name": "Bihar",
    "capital": "Patna",
    "region": "Eastern India",
    "total_places": 15,
    "coordinates": {"lat": 25.0961, "lng": 85.3131},
    "thumbnail_url": "https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "chhattisgarh",
    "name": "Chhattisgarh",
    "capital": "Raipur",
    "region": "Central India",
    "total_places": 11,
    "coordinates": {"lat": 21.2787, "lng": 81.8661},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "goa",
    "name": "Goa",
    "capital": "Panaji",
    "region": "Western India",
    "total_places": 22,
    "coordinates": {"lat": 15.2993, "lng": 74.1240},
    "thumbnail_url": "https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "gujarat",
    "name": "Gujarat",
    "capital": "Gandhinagar",
    "region": "Western India",
    "total_places": 24,
    "coordinates": {"lat": 22.2587, "lng": 71.1924},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "haryana",
    "name": "Haryana",
    "capital": "Chandigarh",
    "region": "Northern India",
    "total_places": 9,
    "coordinates": {"lat": 29.0588, "lng": 76.0856},
    "thumbnail_url": "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "himachal-pradesh",
    "name": "Himachal Pradesh",
    "capital": "Shimla",
    "region": "Northern India",
    "total_places": 20,
    "coordinates": {"lat": 31.1048, "lng": 77.1734},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "jharkhand",
    "name": "Jharkhand",
    "capital": "Ranchi",
    "region": "Eastern India",
    "total_places": 10,
    "coordinates": {"lat": 23.6102, "lng": 85.2799},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "karnataka",
    "name": "Karnataka",
    "capital": "Bengaluru",
    "region": "Southern India",
    "total_places": 26,
    "coordinates": {"lat": 15.3173, "lng": 75.7139},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "kerala",
    "name": "Kerala",
    "capital": "Thiruvananthapuram",
    "region": "Southern India",
    "total_places": 25,
    "coordinates": {"lat": 10.8505, "lng": 76.2711},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "madhya-pradesh",
    "name": "Madhya Pradesh",
    "capital": "Bhopal",
    "region": "Central India",
    "total_places": 22,
    "coordinates": {"lat": 22.9734, "lng": 78.6569},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "maharashtra",
    "name": "Maharashtra",
    "capital": "Mumbai",
    "region": "Western India",
    "total_places": 35,
    "coordinates": {"lat": 19.7515, "lng": 75.7139},
    "thumbnail_url": "https://images.unsplash.com/photo-1570168007204-dfb528c6958f?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "manipur",
    "name": "Manipur",
    "capital": "Imphal",
    "region": "Northeastern India",
    "total_places": 8,
    "coordinates": {"lat": 24.6637, "lng": 93.9063},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "meghalaya",
    "name": "Meghalaya",
    "capital": "Shillong",
    "region": "Northeastern India",
    "total_places": 14,
    "coordinates": {"lat": 25.4670, "lng": 91.3662},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "mizoram",
    "name": "Mizoram",
    "capital": "Aizawl",
    "region": "Northeastern India",
    "total_places": 7,
    "coordinates": {"lat": 23.1645, "lng": 92.9376},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "nagaland",
    "name": "Nagaland",
    "capital": "Kohima",
    "region": "Northeastern India",
    "total_places": 9,
    "coordinates": {"lat": 26.1584, "lng": 94.5624},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "odisha",
    "name": "Odisha",
    "capital": "Bhubaneswar",
    "region": "Eastern India",
    "total_places": 19,
    "coordinates": {"lat": 20.9517, "lng": 85.0985},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "punjab",
    "name": "Punjab",
    "capital": "Chandigarh",
    "region": "Northern India",
    "total_places": 15,
    "coordinates": {"lat": 31.1471, "lng": 75.3412},
    "thumbnail_url": "https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "rajasthan",
    "name": "Rajasthan",
    "capital": "Jaipur",
    "region": "Northern India",
    "total_places": 32,
    "coordinates": {"lat": 27.0238, "lng": 74.2179},
    "thumbnail_url": "https://images.unsplash.com/photo-1599661046289-e31897846e41?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "sikkim",
    "name": "Sikkim",
    "capital": "Gangtok",
    "region": "Northeastern India",
    "total_places": 13,
    "coordinates": {"lat": 27.5330, "lng": 88.5122},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "tamil-nadu",
    "name": "Tamil Nadu",
    "capital": "Chennai",
    "region": "Southern India",
    "total_places": 30,
    "coordinates": {"lat": 11.1271, "lng": 78.6569},
    "thumbnail_url": "https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "telangana",
    "name": "Telangana",
    "capital": "Hyderabad",
    "region": "Southern India",
    "total_places": 21,
    "coordinates": {"lat": 18.1124, "lng": 79.0193},
    "thumbnail_url": "https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "tripura",
    "name": "Tripura",
    "capital": "Agartala",
    "region": "Northeastern India",
    "total_places": 8,
    "coordinates": {"lat": 23.9408, "lng": 91.9882},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "uttar-pradesh",
    "name": "Uttar Pradesh",
    "capital": "Lucknow",
    "region": "Northern India",
    "total_places": 28,
    "coordinates": {"lat": 26.8467, "lng": 80.9462},
    "thumbnail_url": "https://images.unsplash.com/photo-1564507592333-c60657eea523?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "uttarakhand",
    "name": "Uttarakhand",
    "capital": "Dehradun",
    "region": "Northern India",
    "total_places": 19,
    "coordinates": {"lat": 30.0668, "lng": 79.0193},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "west-bengal",
    "name": "West Bengal",
    "capital": "Kolkata",
    "region": "Eastern India",
    "total_places": 24,
    "coordinates": {"lat": 22.9868, "lng": 87.8550},
    "thumbnail_url": "https://images.unsplash.com/photo-1558431382-27e303142255?w=600&auto=format&fit=crop&q=80"
  },

  # 8 Union Territories
  {
    "id": "andaman-and-nicobar",
    "name": "Andaman and Nicobar Islands",
    "capital": "Port Blair",
    "region": "Island Territory",
    "total_places": 14,
    "coordinates": {"lat": 11.7401, "lng": 92.6586},
    "thumbnail_url": "https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "chandigarh",
    "name": "Chandigarh",
    "capital": "Chandigarh",
    "region": "Northern India",
    "total_places": 8,
    "coordinates": {"lat": 30.7333, "lng": 76.7794},
    "thumbnail_url": "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "dadra-nagar-haveli-daman-diu",
    "name": "Dadra and Nagar Haveli and Daman and Diu",
    "capital": "Daman",
    "region": "Western India",
    "total_places": 9,
    "coordinates": {"lat": 20.4283, "lng": 72.8397},
    "thumbnail_url": "https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "delhi",
    "name": "Delhi (NCT)",
    "capital": "New Delhi",
    "region": "Northern India",
    "total_places": 25,
    "coordinates": {"lat": 28.6139, "lng": 77.2090},
    "thumbnail_url": "https://images.unsplash.com/photo-1587474260584-136574528ed5?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "jammu-and-kashmir",
    "name": "Jammu and Kashmir",
    "capital": "Srinagar (Summer) / Jammu (Winter)",
    "region": "Northern India",
    "total_places": 21,
    "coordinates": {"lat": 33.7782, "lng": 76.5762},
    "thumbnail_url": "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "ladakh",
    "name": "Ladakh",
    "capital": "Leh",
    "region": "Northern India",
    "total_places": 17,
    "coordinates": {"lat": 34.1526, "lng": 77.5771},
    "thumbnail_url": "https://images.unsplash.com/photo-1600100397608-f010e422a59e?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "lakshadweep",
    "name": "Lakshadweep",
    "capital": "Kavaratti",
    "region": "Island Territory",
    "total_places": 6,
    "coordinates": {"lat": 10.5667, "lng": 72.6417},
    "thumbnail_url": "https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=600&auto=format&fit=crop&q=80"
  },
  {
    "id": "puducherry",
    "name": "Puducherry",
    "capital": "Puducherry",
    "region": "Southern India",
    "total_places": 12,
    "coordinates": {"lat": 11.9416, "lng": 79.8083},
    "thumbnail_url": "https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=600&auto=format&fit=crop&q=80"
  }
]

print(f"Total States and Union Territories: {len(states_and_uts)}")

with open('data/states.json', 'w', encoding='utf-8') as f:
    json.dump(states_and_uts, f, indent=2, ensure_ascii=False)

print("Saved all 36 States & UTs to data/states.json")
